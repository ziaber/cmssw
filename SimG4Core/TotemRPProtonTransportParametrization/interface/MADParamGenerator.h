...
class MADParamGenerator
{
  public:
    ...
    int GenerateLostParticleData(const Parametisation_configuration &conf);
    ...
    void IdentifyAperturesForAll();
    void IdentifyApertures(int id);

    bool CheckParamConfId(int id);
    bool CheckApertureConfId(int param_id, int apreture_id);

    void TrainAndAddApertures(const Parametisation_configuration &conf, LHCOpticsApproximator &approximator, TFile *f);
 ...
 // private:
    int BuildSample(const Parametisation_configuration &conf, std::string sample_file_name, bool recloss=false, bool compare_apert = false);
    ...
    TTree *GetSamplesTree(const Parametisation_configuration &conf, std::string sample_file_name);
