...
/**
 * Raw VFAT frame (as sent by electronics).
**/
...
class VFATFrame
{
  ...
  public:
    ...
    /// Checks the fixed bits, returns true if they're as they should be.
    bool checkFootprint() const
    {
      return ((data[9] & 0xF000) == 0xE000) && ((data[10] & 0xF000) == 0xC000) && ((data[11] & 0xF000) == 0xA000);
    }

    /// Checks the validity of CRC.
    /// Returns true if the CRC is OK.
