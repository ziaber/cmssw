...
void DDDTotemRPContruction::buildDetGeomDesc(DDFilteredView *fv, DetGeomDesc *gd)
{
	...
	do {
      ...
      std::cout<<"TotemRPDetId(arm, station, rp, detector) "<<TotemRPDetId(arm, station, rp, detector).detectorDecId()<<", "<<TotemRPDetId(arm, station, rp, detector).decToRawId(otemRPDetId(arm, station, rp, detector).detectorDecId())<<std::endl;
